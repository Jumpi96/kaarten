        ...
        Some(message) => match message.as_object() {
            Some(message_obj) => match message_obj.get("text") {
                Some(text) => match text.as_str() {
                    ...
                    Some(text) if text.starts_with("/list") => handlers::list_handler(message, false).await,
                    Some(text) if text.starts_with("/LSlist") => handlers::ls_handler(message, false).await,
                    Some(text) if text.starts_with("/LSduplicated") => handlers::ls_handler(message, false).await,
                    Some(text) if text.starts_with("/duplicated") => handlers::list_handler(message, true).await,
